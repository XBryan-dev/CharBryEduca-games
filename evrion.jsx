...
/*  Single-file app. Content lives in window.storage (shared, so the  */
/*  one admin's edits are what every visitor sees). Quiz results are  */
/*  shareable via a compact token in the URL.                         */
/* ------------------------------------------------------------------ */

const STORAGE_KEY = "evrion_content_v1";
const ADMIN_SESSION_KEY = "evrion_admin_session";
const ADMIN_PASSCODE = "evrion2026"; // change this, then redeploy
...
async function persistContent(content) {
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify(content), true);
  } catch (e) {
    console.error("Could not save EVRION content", e);
  }
}

async function loadAdminSession() {
  try {
    const res = await window.storage.get(ADMIN_SESSION_KEY, false);
...
async function setAdminSession(v) {
  try {
    if (v) await window.storage.set(ADMIN_SESSION_KEY, "1", false);
    else await window.storage.delete(ADMIN_SESSION_KEY, false);
  } catch (e) {
    /* ignore */
  }
}
...
function ResultView({ category, personality, pct, onShare, onReplay, onCategories, shareStatus }) {
  return (
    <div>
    ...
    <div>
      ...
      <div className="evrion-scroll">
        ...
        <div className="evrion-field">
          <label className="evrion-label">Passcode</label>
          <input
            type="password"
...
function AdminDashboard({ content, setContent, onLogout, onBack }) {
  ...
  return (
    <div>
      ...
      <div className="evrion-scroll">
        ...
        {tab === "Categories" && (
          <CategoriesTab content={content} save={save} setModal={setModal} />
        )}
        {tab === "Questions" && (
          <QuestionsTab content={content} save={save} setModal={setModal} />
        )}
        {tab === "Traits" && <TraitsTab content={content} save={save} setModal={setModal} />}
        {tab === "Personalities" && (
          <PersonalitiesTab content={content} save={save} setModal={setModal} />
        )}
      </div>
...
export default function EvrionApp() {
  const [content, setContent] = useState(null);
  const [loading, setLoading] = useState(true);
  const [view, setView] = useState("home");
  const [category, setCategory] = useState(null);
  const [result, setResult] = useState(null); // { personality, pct, categoryId }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (token) {
        ...
        if (decoded) {
          const personality = c.personalities.find((p) => p.id === decoded.p);
          if (personality) {
      ...
      const isAdmin = await loadAdminSession();
